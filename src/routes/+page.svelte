<script lang="ts">
  ...
  import { Search, Globe2, ChartCandlestick, Coins, CoinsIcon } from 'lucide-svelte';
  ...
  let filteredAirdrops = $derived.by(() => {
    ...
    if (activeCategory !== 'all') {
      results = results.filter((airdrop) =>
        activeCategory === 'official' ? airdrop.isOfficial : !airdrop.isOfficial
      );
    }
...
<div class="flex min-h-screen flex-col">
  ...
  <main class="relative flex-1 bg-[#020817]">
  ...
  <footer class="mt-24 bg-[#FF0000] py-1.5">
    <div class="container mx-auto px-4 text-center">
      <p
        class="flex items-center justify-center gap-1.5 text-sm font-medium text-white"
      >
        Brought to you by
        <a
